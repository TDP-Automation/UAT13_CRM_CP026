...
Call EnviarOrden()
If ucase(str_metodo_entrega) <> "DELIVERY" Then
...
Sub ParametrosCambio()
	...
	For Iterator = 0 To count-1 Step 1
		x = JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Actualizar Atributos de_2").JavaList("Motivo:").GetItem ("#"&Iterator)
		If x = str_TipodeCambio Then
			Reporter.ReportEvent micPass, "existe cambio de plan","yes"
...
'			End If
	...
	If DataTable("e_Compromiso", dtLocalSheet) <> "NO"  Then
		...
		wait 5
		dim Itera , fil, s, v
		fil = JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar Configuración").JavaTable("Mostrar atributos:_2").GetROProperty("rows")
		fil = fil-1
		For Itera = fil To 0 step -1
			JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar Configuración").JavaTable("Mostrar atributos:_2").SelectRow ("#"&Itera)
			s = JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar Configuración").JavaTable("Mostrar atributos:_2").GetCellData("#"&Itera, "#1")
			v = Instr(1,s,"10011")	    
		    If v <> "0" Then
		    	
		    	JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar Configuración").JavaTable("Mostrar atributos:_2").ClickCell "#"&Itera,"#1"
		    	wait 1
		    	JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar Configuración").JavaTable("Mostrar atributos:_2").SetCellData "#"&Itera, "#1",DataTable("e_Compromiso", dtLocalSheet)
		    	JavaWindow("Ejecutivo de interacción").CaptureBitmap RutaEvidencias() & "18meses.png", True
				imagenToWord "Seleccionamos 18 mese de compromiso",RutaEvidencias() & "18meses.png"
		    	 Exit for
		    End If
...
'	    JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar Configuración").JavaTable("Mostrar atributos:_2").SelectRow ("#"&Iterator)
'		j = JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar Configuración").JavaTable("Mostrar atributos:_2").GetCellData("#"&Iterator, "#1")
'	    h = Instr(1,j,"Periodo de compromiso del Equipo")
'	    If h <> 0 Then
'	    	JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar Configuración").JavaTable("Mostrar atributos:_2").DoubleClickCell "#"&Iterator, "#2", "LEFT"
'	    	Exit for 
'	    End If
...
Sub TipoEnvio()
    While JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar dirección de").JavaRadioButton("En tienda").Exist = false
    	wait 1
    Wend
	Select Case ucase(str_metodo_entrega)
		Case "EN TIENDA"
		...
		Case "DELIVERY"
				...
				wait 1
				JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar dirección de_2").JavaEdit("TextFieldNative$1").Set "109912874"
				wait 1
...
Sub GestionLogistica()
	...
	JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Buscar: Orden").JavaEdit("TextFieldNative$1").Set str_NroOrden									
	wait 1
...
Sub EmpujeOrden()
	If DataTable("e_Tipo_Data", dtLocalSheet) = "DATA LOGICA" Then
		...
		JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Buscar: Grupo de órdenes").JavaEdit("TextFieldNative$1").Set str_NroOrden
		wait 2
...
'		j = JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar Configuración").JavaTable("Mostrar atributos:_2").GetCellData("#"&Iterator, "#1")
'	    MsgBox j
'	    If j="Periodo de compromiso del Equipo" Then
'	    	
'	    	JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar Configuración").JavaTable("Mostrar atributos:_2").ClickCell "#"&Iterator,"#1"
'	    	wait 1
'	    	JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar Configuración").JavaList("0,00").Select "18"
'	    	
'
'	    	 Exit for
'	    End If
